...
jugador.
"""
def movimiento_valido(x, y, movimientos_otro_jugador, n=3):
    if x > n or y > n:
        return False
...
def test_movimiento_columna_fuera_tablero(n=3):
    movimientos_otro_jugador={}
    x= 1
    y= n+1
    assert False == movimiento_valido(x,y,movimientos_otro_jugador)
    
def test_movimiento_fila_fuera_tablero(n=3):
    movimientos_otro_jugador={}
    x= n+1
    y= n
    assert False == movimiento_valido(x,y,movimientos_otro_jugador)
    
def test_movimiento_fila_y_columna_fuera_tablero(n=3):
    movimientos_otro_jugador={}
    x= n+1
    y= n+1
    assert False == movimiento_valido(x,y,movimientos_otro_jugador)
